...
class TutorController extends Controller
{
    ...
    public function oneOnOneSlot(Request $id) 
    {
        // Find tutor (role 2 or 4, active)
        ...
        ->map(function ($slot) use ($categories) {
            ...
            $slot->class_date_formatted   = $classDateObj->format('jS F Y');
        ...
        if($slots->isNotEmpty())    
        {
            return jsonResponse(true, 'Fetching slots for first 7 days of current month', [
                'current_month'     => $today->format('F Y'), // March 2026
                'available_dates'   => $dates,                // ["2026-03-01", "2026-03-02", ...]
                'slots'             => $slots,                // grouped by date or flat list
            ]);
        }
